	   ...
	   //==> 주민번호 유효성 check 는 이해정도로....
		...
		$( function() {
			$('#password2').on('focusout', function() {
				...
				if(pw != pw2) {
					$('#password2').parent().append('<p id="pw">암호가 다릅니다.</p>').css('color','red');
				}
			})
		})
...
//					$('.col-sm-4 p').remove();
					...
					$.ajax({
						...
						headers : {
							...
							if( len.length < 5){
								$('#userId').parent().attr('class','col-sm-4 has-warning');
								$('#userId').parent().append('<p id="id">아이디는 5글자 이상 입력.</p>').css('color','red');
								
								
							}else {
								
								if(JSONData == true) {
								$('#userId').parent().attr('class','col-sm-4 has-success');
								$('#userId').parent().append('<p id="id">사용가능한 아이디입니다.</p>').css('color','green');
							
								}else {
									$('#userId').parent().attr('class','col-sm-4 has-error');
									$('#userId').parent().append('<p id="id">이미 사용중인 아이디입니다.</p>').css('color','red');
